...
export async function renderNavigator(system: System, theme: Theme = defaultTheme): Promise<string> {
  ...
  for (const id of ids) {
    ...
    for (const n of mods[id]!.interior.nodes) {
      // A `module` node is a link; a `method` node with a resolved `ref` is also a
      // navigable edge (a call into a known class's method).
      const ref = n.kind === "module" ? n.ref : n.kind === "method" ? n.ref : undefined;
