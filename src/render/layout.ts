...
export type NodeKind =
  | "function" | "method" | "branch" | "merge" | "loop" | "while" | "foreach" | "try" | "with" | "assert" | "throw" | "rethrow" | "break" | "continue" | "return" | "yield" | "effect" | "const" | "module"
  | "select" | "array" | "collection" | "comprehension" | "itercomp" | "index" | "slice" | "await" | "globalRef" | "selfRef"
  | "state" | "stateGet" | "stateSet" | "attrGet" | "attrSet" | "indexSet" | "delIndex" | "delAttr" | "unpack" | "broadcast"
  | "boundary";
...
function readBack(
  ...
  for (const n of mod.interior.nodes) {
    ...
    // A `module` node always carries a ref; a `method` node carries one only when
    // its receiver's class was resolved — either makes the node a navigable link.
    if (n.kind === "module") irNodeRef.set(n.id, n.ref);
    else if (n.kind === "method" && n.ref) irNodeRef.set(n.id, n.ref);
  }
